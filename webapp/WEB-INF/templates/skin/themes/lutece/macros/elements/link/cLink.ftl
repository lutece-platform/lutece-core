...
<a href="${href!}" <#if id!=''> id="${id!}"</#if> <#if class !='' >class="${class!}"</#if><#if title!=''> title="${title!}<#if target='_blank'> #i18n{theme.newWindowLink}</#if>"</#if><#if target!=''> target="${target}"</#if><#if params!=''> ${params}</#if> > 
<#if nestedPos!='after'><#nested></#if>
<#if label!=''><span class="link-label">${label!}</span></#if> 
<#if nestedPos='after'> <#nested></#if>
<#if target='_blank' && label=''><span class="visually-hidden visually-hidden-focusable"><#if title!=''>${title!}<#else>${label}</#if> #i18n{theme.newWindowLink}</span></#if>
