...
@param - dismissLabel - string - optional - permet de définir le label du bouton de fermerture de la modal (par défaut: '#i18n{theme.labelClose}')
...
@param - params - string - optional - permet d'ajouter des parametres HTML à la modal
 -->
<#macro cModal title id size='lg' static=false pos='centered' role='' scrollable=false dismissible=true dismissLabel='#i18n{theme.labelClose}' footer='' titleLevel=1 class='' params='' deprecated...>
    ...
    <div class="modal-dialog <#if pos!=''>modal-dialog-${pos} </#if> <#if scrollable>modal-dialog-scrollable</#if><#if size!=''>modal-${size}</#if>"<#if role !=''> role="${role}"</#if>>
        <div class="modal-content">
            <div class="modal-header">
                ...
                <#if dismissible>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="#i18n{theme.labelClose}">
                    </button> 
                </#if>
            </div>
