...
<a href="${href!}" <#if id?has_content> id="${id!}"</#if> <#if class?has_content >class="${class!}"</#if><#if title?has_content> title="${title!}<#if target='_blank'> #i18n{portal.theme.newWindowLink}</#if>"</#if><#if target?has_content> target="${target}"</#if><#if params?has_content> ${params}</#if> > 
<#if nestedPos!='after'><#nested></#if>
<#if label?has_content><span class="link-label">${label!}</span></#if> 
<#if nestedPos='after'> <#nested></#if>
<#if target='_blank' && !label?has_content><span class="visually-hidden visually-hidden-focusable"><#if title?has_content>${title!}<#else>${label}</#if> #i18n{portal.theme.newWindowLink}</span></#if>
