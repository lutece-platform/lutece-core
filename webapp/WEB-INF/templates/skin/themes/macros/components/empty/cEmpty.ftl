...
<#if !img?has_content><div class="icon<#if iconClass?has_content> ${iconClass}</#if>"><@cIcon name='${iconName}' params=paramsAttr1 /></div><#else>	<div class="illustration<#if imgClass?has_content> ${imgClass}</#if>"><img src="${img}" height="128" alt=""></div></#if>
...
<p class="actions<#if actionClass?has_content> ${actionClass}</#if>">
	<a href="${actionUrl}" class="btn btn-${actionBtn}">
        <#if actionIcon?has_content><@icon prefix='ti ti-' style='${actionIcon}' /></#if> ${actionTitle}
    </a>
</p>
</#if>
</div>
