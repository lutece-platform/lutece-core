<@row>
    ...
    <@box color='success' collapsed=true>
			<#assign headerTitle>
				#i18n{portal.rbac.assign_users.role.name} :<strong title="#i18n{portal.rbac.assign_users.role.description} : ${role.description} "> ${role.key}</strong> <small class="spaced"><@icon style='user' />&nbsp;#i18n{portal.rbac.assign_users.role.number_user} : <@tag>${assigned_users_number}</@tag></small>
			</#assign>
			<@boxHeader title='${headerTitle}' boxTools=true>
