...
<select id="${id}" name="${name}" class="<#if size?has_content>form-control form-control-${size}</#if><#if class?has_content> ${class}<#else> form-select</#if>" <#if (multiple &gt; 0)>multiple size="${multiple}"</#if><#if (tabIndex &gt; 0)> tabindex="${tabIndex}"</#if><#if params?has_content> ${params}</#if><#if title?has_content> title="${title}"</#if><#if mandatory> required</#if><#if disabled> disabled</#if>>
