...
<div id="${id}" class="lutece-autocomplete dropdown form-group form-floating " data-itemTitleFieldNames=${itemTitleFieldNames} data-suggestionsUrl="${suggestionsUrl}" data-suggestionsPath="${suggestionsPath}" data-itemValueFieldName=${itemValueFieldName} data-minimumInputLength=${minimumInputLength} data-itemDescriptionFieldNames=${itemDescriptionFieldNames} data-itemTagsFieldNames=${itemTagsFieldNames} data-copyFields=${copyFields} data-emptyLabel="${emptyLabel}" data-currentValue="${currentValue}" data-currentLabel="${currentLabel}" data-suggestionItemClass='["list-group-item", "p-3"]' data-titleClass='["mb-0", "fw-bolder"]' data-descriptionClass='["text-muted", "mb-0"]' data-tagClass='["badge", "bg-blue-lt", "me-1"]' data-loaderIconClasses='{"loading": ["ti-loader-2", "icon-rotate"], "error": ["ti-zoom-exclamation", "text-danger"], "search": ["ti-search"]}' data-emptyClass='["list-group-item", "p-3", "text-muted", "text-center"]' data-searchLabel="${searchLabel}" data-allowFreeText="${allowFreeText?c}">
  <@formGroup formStyle='floating' class='' labelFor='${id}' labelKey='${searchLabel}' mandatory=mandatory>
    <@inputGroup>
      <#local paramsAttr1>role="combobox" autocomplete="off" aria-expanded="false" aria-owns="${id}-list-container" aria-autocomplete="list" aria-describedby="${id}-lutece-autocomplete-assistiveHint"</#local>
      <@input type='text' name="${name}" class="lutece-autocomplete-search-input w-75" value="${currentValue!}"  id="${id}-input" params=paramsAttr1 />
      <span role="button" class="input-group-text lutece-autocomplete-remove <#if !currentValue?has_content>d-none</#if> text-danger">
...
new LuteceAutoComplete(document.getElementById(`${id}`)<#if additionnalRequestParamInputId?has_content>, { additionalParamElement: document.getElementById(`${additionnalRequestParamInputId}`) }</#if>);
</script>
</#macro>