...
<#macro accordionHeader id='' title='' parentId=parentId childId=childId boxTools=false params='' headerIcon='' >
	<h2 class="accordion-header" id="${childId}-header"<#if params!=''> ${params}</#if>>
		<div class="accordion-button<#if aClass!=''> ${aClass}</#if>" type="button" data-bs-toggle="collapse" data-bs-target="#${childId}" aria-expanded="${expanded}" aria-controls="${childId}">
		<#if headerIcon!=''><@icon style=headerIcon /></#if><span class="ms-2">${title}</span>
			<#local nested><#nested></#local>
	<#if nested?has_content && boxTools><div class="position-absolute end-0 me-5" onclick="event.stopPropagation();">${nested}</div></#if>
		</div>
	</h2>
</#macro>