...
<div class="card card-sm mb-3 box-widget p-2"  draggable='true' <#if id!=''> id="${id}" data-id="${id}"</#if><#if params!=''> ${params}</#if>>
	<div class="card-body">
		<div class="row align-items-center">
			...
			<div class="col">
			<#if url!=''><a class="card-link fs-5 fw-bold" href="${url}"></#if>
			<${titleLevel} class=""><span class="counter">${title}</span><#if unit!=''> ${unit}</#if></${titleLevel}>
			<div class="text-muted fw-normal">${text}</div>
			<#if url!=''></a></#if>
			</div>
		</div>
	</div>
