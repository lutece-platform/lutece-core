...
-->
<#macro dropdownItem class='' href='' target='' title='' id='' params='' disabled=false>
<li>
	<a href="${href}" class="dropdown-item<#if class!=''> ${class}</#if> <#if disabled>disabled</#if>" title="${title}"<#if id!=''> id="${id}"</#if><#if params!=''> ${params}</#if><#if target!=''> target="${target}"</#if>>${title}</a>
</li>
</#macro>