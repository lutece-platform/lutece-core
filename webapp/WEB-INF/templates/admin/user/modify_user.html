<#assign boxtitle>
<@icon style='user/> ${user.lastName} ${user.firstName}
</#assign>
<@row>
	<@columns>
		<@box color='success'>
				...
				<@boxBody>
					<@tabList>
						<li class="active">
								<@link href='jsp/admin/user/ModifyUser.jsp?id_user=${user.userId}'>#i18n{portal.users.actions.labelUser}</@link>
						</li>
					...
					<@row>
							<#if attributes_list?has_content><@columns sm=4><#else><@columns sm=6></#if>
							...
							<#if attributes_list?has_content><@columns sm=4><#else><@columns sm=6></#if>
									<@fieldSet legend='#i18n{portal.users.modify_user.labelAttributes}'>
											...
											<@formGroup labelFor='status' labelKey='${i18n("portal.users.modify_user.labelStatus")}' rows=2>
													<@select name="status">
															<#if user.status == 0 >
																	<option value="0" selected >#i18n{portal.users.modify_user.statusLabelEnabled}</option>
															...
															<#else>
																	...
																	<option value="1" selected >#i18n{portal.users.modify_user.statusLabelDisabled}</option>
															</#if>
													</@select>
											</@formGroup> 
