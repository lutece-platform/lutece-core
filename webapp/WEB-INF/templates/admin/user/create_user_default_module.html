			...
			<@boxBody>
				<@tform method='post' name='create_user' action='jsp/admin/user/DoCreateUser.jsp' params='enctype="multipart/form-data"'>
						...
						<@columns md=4 lg=4>
							...
							<@formGroup labelKey='#i18n{portal.users.create_user.labelLastName}' labelFor='last_name' rows=2 mandatory=true>
								<@inputGroup>
									...
									<@input type='text' id='last_name' name='last_name' value='' maxlength=100 />
								</@inputGroup>
							</@formGroup>
							<@formGroup labelKey='#i18n{portal.users.create_user.labelFirstName}' labelFor='first_name' rows=2 mandatory=true>
								<@inputGroup>
									...
									<@input type='text' id='first_name' name='first_name' value='' maxlength=100 />
								</@inputGroup>
							</@formGroup>
						...
						<@columns md=4 lg=4>
							...
							<@formGroup labelKey='#i18n{portal.users.create_user.labelAccessCode}' labelFor='access_code' rows=2 mandatory=true>
								<@inputGroup>
									...
									<@input type='text' id='access_code' name='access_code' value='' maxlength=100 />
								</@inputGroup>
							</@formGroup>
							<@formGroup labelFor='first_password' labelKey='#i18n{portal.users.create_user.labelFirstPassword}' helpKey='${i18n("portal.users.message.password.minimumPasswordLength",minimumPasswordSize)}' mandatory=true rows=2>
								<@inputGroup>
									<@inputGroupItem type='text'>
										<@icon style='lock' />
									</@inputGroupItem>
									<@input type='password' id='first_password' name='first_password' maxlength=100 value='' />
									<@inputGroupItem>
							...
							<@formGroup labelKey='#i18n{portal.users.create_user.labelSecondPassword}' labelFor='second_password' rows=2 helpKey='#i18n{portal.users.create_user.passwordComplexity}' mandatory=true>
								<@inputGroup>
									...
									<@input type='password' id='second_password' name='second_password' maxlength=100 value='' />
								</@inputGroup>
							</@formGroup>
							<@formGroup rows=2>
								<@progressBar />
						...
						<@fieldSet legend='#i18n{portal.users.create_user.labelComplementaryAttributes}'>
							...
							<#if displayFieldset( user_levels attributes_list )>
								<#if (user_levels?has_content)>
									...
									<@formGroup labelKey='#i18n{portal.role.create_role.labelWorkgroupKey}' rows=2 mandatory=true>
					...
					<@row>
						<@columns md=4 lg=4>
							<@formGroup rows=2>
								<@button size='' type='submit' buttonIcon='check' title='#i18n{portal.users.manage_users.buttonLabelAddUser}'  />
...
$(document).ready(function(){	
	$("#generate_password").click(function(){
		var pwd = $.password(${randomPasswordSize}, true);
		$("#first_password").val(pwd);
		$("#second_password").val(pwd);
		$("#first_password").trigger("propertychange");
    });
