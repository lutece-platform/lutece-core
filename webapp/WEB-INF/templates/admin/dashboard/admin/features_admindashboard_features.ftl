...
<@tabPanel id='assign_features_groups'>
    <p>
			<em>#i18n{portal.features.dispatch_features.featureDescription}</em>
    </p>
    <@tabContent>
	...
	<@columns>
		<@box color='danger'>
			<#if groupDisplay.feature_group.order !=0>
				<#assign title>
					#i18n{${groupDisplay.feature_group.labelKey}} <@link name='${groupDisplay.feature_group.id?if_exists}'>&nbsp;</@link>
				</#assign>
			<#else>
				<#assign title>
					...
					<#list groupDisplay.feature_list?sort_by('order') as right>
					  <#if right.url?has_content>
						<tr>
						  ...
						  <td>
							<@tform method='post' type='inline' action='jsp/admin/features/DoDispatchFeature.jsp'>
