...
<#macro coreAdminCSSLinks>
    <link href="css/admin/style/carbon/carbon-components.min.css"  rel="stylesheet" > <!-- provided by core -->
    ...
    <script src="css/admin/style/carbon/carbon-components.min.js" ></script>
        ...
        <#if warnings?size &gt; 0 >
            <div class="${warnings_class}" id="messages_warnings_div" >
                ...
                <#list warnings as warning >
                    <i class="fa fa-info-circle"></i> ${warning.message} <br>
                </#list>
            </div>
        </#if>
    </#if>
...
<#macro tabList type='tabs' id='' params=''>
...
<#-- color: btn-default[bootstrap4 : btn-secondary]/btn-primary/btn-success/btn-warning/btn-danger/btn-info/bg-purple/bg-blue/bg-navy/bg-teal/bg-maroon/bg-black/bg-gray/bg-olive/bg-lime/bg-orange/bg-fuchsia -->
...
<#macro button name='' id='' type='button' size='' color='' style='btn-flat' class='' params='' value='' title='' tabIndex='' showTitle=true showTitleXs=true showTitleSm=true showTitleMd=true showTitleLg=true buttonIcon='' disabled=false iconPosition='left' dropdownMenu=false cancel=false>
	<#if cancel>
		<#assign buttonColor = 'btn-default' />
	<#elseif !cancel && color=''>
		<#assign buttonColor = 'btn-primary' />
	<#else>
	...
	<button class="<#if style!='close'>btn</#if><#if size!='' && style!='btn-box-tool'> btn-${size}</#if><#if buttonColor!='' && style!='btn-box-tool'> ${buttonColor}</#if><#if style!=''> ${style}</#if><#if dropdownMenu> dropdown-toggle</#if><#if class!=''> ${class}</#if>" type="${type}"<#if title!=''> title="${title}"</#if><#if name!=''> name="${name}"</#if><#if id!=''> id="${id}"</#if><#if value!=''> value="${value}"</#if><#if params!=''> ${params}</#if><#if disabled> disabled</#if><#if dropdownMenu> data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"</#if><#if cancel> formnovalidate</#if>>
...
<#-- color: btn-default/btn-primary/btn-success/btn-warning/btn-danger/btn-info/bg-purple/bg-blue/bg-navy/bg-teal/bg-maroon/bg-black/bg-gray/bg-olive/bg-lime/bg-orange/bg-fuchsia -->
...
<div class="alert alert-${class}"<#if id!=''> id="${id}"</#if><#if params!=''> ${params}</#if>>
	<#if dismissible>
...
<div class="small-box<#if color!=''> ${color}</#if>"<#if id!=''> id="${id}"</#if><#if params!=''> ${params}</#if>>
	<div class="inner">
...
<#macro badge id='' params=''>
	<span class="badge"<#if id!=''> id="${id}"</#if><#if params!=''> ${params}</#if>>
		<#nested>
	</span>
</#macro>
