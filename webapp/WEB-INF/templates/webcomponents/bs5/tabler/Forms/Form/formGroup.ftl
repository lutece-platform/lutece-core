...
-->
<#macro formGroup id='' formStyle='horizontal' groupStyle='' class='' rows=1 labelKey='' labelFor='' labelId='' labelClass='' helpKey='' mandatory=false hideLabel=[] collapsed=false params='' deprecated...>
...
<div class="form-group<#if formStyle='horizontal'> mb-3 row</#if><#if class!=''> ${class?trim}</#if><#if validation?? && validation!=''> ${validation}</#if>"<#if id!=''> id="${id}"</#if><#if params!=''> ${params}</#if>>
	<#local displayLabelClass = displaySettings(hideLabel,'inline-flex') />
	<#if rows=1>
		<#if labelKey!='' && formStyle='horizontal'>
			<#local labelClass += ' col-sm-12 col-lg-3 col-form-label text-right'>
	...
	<#if labelKey!=''>
		<@formLabel class=labelClass?trim labelFor=labelFor labelId=labelId labelKey=labelKey hideLabel=hideLabel mandatory=mandatory />
	</#if>
	<#if formStyle !='inline'>
		<div class="${divClass}">
	</#if>
